...
This analysis will  be focsed on some basic exploratory plots comparing some match data across time and between mens and womens matches.
...
points payed per set is about even across mens and womens matches, with medians of about 60 points per set.
```{r}
...
When looking at points per game again the median is about the same across mens and womens matches, but mens sets tend to have more outliers.
...
matches %>%
  ...
  filter(game_winner != 0,
         game_seq >= 3) %>%
  ggplot(aes(x = as.factor(set_no), y = game_seq, fill = as.factor(year))) + 
  geom_boxplot(alpha = .80) + 
  scale_fill_npg() + 
  coord_flip() +
  facet_wrap(vars(category), scales = 'free') + 
...
Serve speeds differ across mens and womens matches, with the median meas serve being about 108 mph and womens median being about 95 mph.
```{r}
...
matches %>%
  inner_join(., points, "match_id") %>% str()
  select(match_id, p1first_srv_in, p2first_srv_in) %>%  na.omit()
  mutate(cuml_wins = row_number()) %>%
  inner_join(., points, "match_id") %>% 
  filter(year == 2021,
         game_length_mins > 400)
```
...
wimb <- 
  wimbledon_matches %>% 
  select(match_id, year, slam, match_num, player1, player2) %>% na.omit()
